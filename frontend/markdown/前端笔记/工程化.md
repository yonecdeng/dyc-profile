...
- pet：pet 组件市场 [PET·组件市场用户文档](https://docs.corp.kuaishou.com/d/home/fcAD0Xs3HyawpSyh5Q3R-sqSb)
...
| 依赖管理   | ❌ 多个仓库都有自己的 node_modules，存在依赖重复安装情况，占用磁盘内存大。 | ✅ 多项目代码都在一个仓库中，相同版本依赖提升到顶层只安装一次，<font color="red">节省磁盘内存。</font>但是会蹭其它项目的依赖？？。 |
...
[Turborepo 中使用 remote cache](https://docs.corp.kuaishou.com/k/home/VE0dSNlz-4UM/fcACs2AT3A7NZtWUks_Onl72e)
...
在活动中，特殊字体一般出现于大标题中，所以通过调用[@pet/base.heading](https://pet-web.corp.kuaishou.com/gallery/detail/?name=@pet/base.heading)组件，可以更好的统一管理特殊字体。**只需统一收口关注** g-font-han 这个类的 font-family 的定义。
...
- 对于进入页面，加载的 loading，我们通常会统一处理，例如云转场。

- 对于进入页面后拉取数据或加载资源，我们有几种选择：

- - 展示 loading 或 空状态的会场。

  - 利用统一转场（如云转场），延后转场结束时间至数据返回。

  - 展示默认数据，通常不推荐采用这种方式，原因为：

  - - 会展示错误的信息至用户端。
    - 会造成重复渲染，可能延后 fmp 统计。

- 对于用户操作后请求发出但未返回时，**通常**不做 loading 状态的处理，但具体还需要和会场PM，交互与设计沟通。此外用户操作还有一些需要注意的点：

- - 重复操作保护与幂等性：对于一些操作（例如抽奖等）会造成数值变化的，一般会进行重复操作保护，以防用户重复点击，弱网等导致误操作的情况出现。简单的处理方法可以设置一个标志位去过滤操作或禁用用户操作。

- 对于数据本身来说，每个请求都会有一个 **result** 值，代表不同的含义。
...
- - 一些常见的值为：
  ...
  - - 10001 - 活动未开始
    ...
    - - 对于这些常见异常，我们通常采用跳转到错误兜底页的方式处理，并且已经封装好了一个组件：[PET 组件市场 - sf.error-page](https://pet-web.corp.kuaishou.com/gallery/detail/?name=@pet/sf.error-page)。
  ...
  - - 未登录状态的 **result** 值
    ...
    - 对于未登录异常，我们通常会调起登陆弹窗，也就是调用 yoda bridge: [Yoda文档站 - login](https://yoda.corp.kuaishou.com/docs/bridge/KUAISHOU/kwai/login.html) 。

    - 由于登陆弹窗是可关闭的弹窗，因此我们还需要考虑调起登陆弹窗后，用户再次手动操作关闭登陆弹窗的场景。一般的处理方式为：

    - - 会场展示未登录兜底状态。
      - 全屏幕蒙层，当蒙层点击时再次调起登陆弹窗，直至登陆成功。

    - 成功登陆后，需要考虑恢复现场：

    - - 通常的做法为重新调用会场主接口，刷新数据，但这种做法有一定对数据模型的设计和治理的要求。
      - 重新加载页面(webview)。这种做法简单，但是用户体验极差，可以考虑研发初期采用这种方式。

  - 其余异常状态处理

  - - 我们将业务内的异常状态分为几种

    - - 重要数据的异常，即影响主流程，影响内容展示和数据准确性的。
      - 用户操作流程中的异常。
      - 弹窗/下拉窗口的异常。

    - 对于重要数据的异常，推荐直接跳转兜底页，以免对用户展示错误信息和产生错误数据，例如主接口拉取失败。

    - 对于用户操作流程中的异常，通常可以直接 Toast 提示，以免出现用户点击导致页面之间跳到兜底，这样同样会降低用户体验。

对于弹窗/下拉窗口等中的异常，通常仅在弹窗/下拉窗口中展示异常态，我们已经封装好了一个组件： [PET 组件市场 - sf.empty-status](https://pet-web.corp.kuaishou.com/gallery/detail/?name=@pet/sf.empty-status)。
...
归根结底，落到web上之后，目前无非 js+css、svg、canvas、webgl这几种方式。
...
1. 代码实现：使用CSS或者JS控制实现（css动画、canvas动画、svg动画）
2. 关键帧/序列帧动画：主要包括APNG、Lottie和序列帧动画、关键帧动画、透明视频
3. 骨骼动画（3D、Spine）



- 逐帧动画（序列帧）：一般使用 CINEMA 4D 进行制作，导出为 APNG/视频/GIF等

  - 好处：灵活性强，因为每一帧可以单独制作，理论上可以实现所有想要的效果；不需要实时计算，解码后对CPU / GPU友好
  - 坏处：每一帧是单独的图片，体积和运行时的内存占用都比较大

- [Lottie](https://lottiefiles.com/what-is-lottie)：一种跨平台使用的、基于JSON的、矢量动画格式，一般使用 Adobe After Effects 进行制作，Lottie 提供了相应的插件导出相应的格式

  - 好处：体积小；因为是矢量动画，可以任意缩放；跨平台
  - 坏处：多图层的情况，设计师工作量很大；人物、动物等用 Lottie 很难实现的比较逼真；运行时可能对 CPU / GPU 有压力

- 骨骼动画：一种用于在计算机图形和动画中模拟运动的技术。它通过使用虚拟骨骼系统来模拟物体的运动和变形。可以看 @王浩宇写的：[骨骼动画](https://docs.corp.kuaishou.com/d/home/fcACDcbxKvtg810CFRLq2KEbO)。一般有 3D 和 2D 骨骼动画两种类型，3D 骨骼动画一般用 3ds Max 来制作，2D 骨骼动画一般用 Spine Editor 制作。

  - 好处：模拟人物、动物等的动作更精准、逼真
  - 坏处：使用场景比较局限，主要用来实现有「骨骼」的物体；设计师的学习成本比较高，目前部分设计师不具备这个能力；运行时可能对 CPU / GPU 有压力

- 粒子系统：通常用于表现<font color="red">大量单位</font>>以一定规则变化的效果，比如雪、落叶和流星拖尾等等。**粒子动画是由在一定范围内随机生成的大量粒子产生运动而组成的动画，被广泛运用于模拟天气系统、烟雾光效等方面。在电商平台的微型游戏化场景中，粒子动画主要 用于呈现在能量收集、金币收集时的特效。**

  一般的游戏开发工具都有粒子系统，我们的设计师一般使用 unity 来生产粒子动效

  - 好处：粒子适用的场景，动效设计师的生产成本很低，如果用lottie之类的方式来实现成本非常高
  - 坏处：目前没有粒子生产工具的导出产物可以在 web 平台使用，一般需要对导出产物做转换，或自己实现/使用开源的 web 平台工具，但效果会有一定阉割；运行时可能对 CPU /  GPU 有压力
...
3. 视频方案(mp4)：采用H264编码，相比动图方案，有很高的压缩率，硬件解码效率很高，缺点很明显，无法支持透明背景，在某些对透明背景有强诉求的场景就不太适用了
...
- 普通CSS (关键帧CSS)
  - 在这个面板下, 平台将该图层动画转换为逐帧CSS, 并且可以根据你的需求选择不同的关键帧帧率, 因为在关键帧之间使用了线性补间, **一般来说, 将帧率设置为10就可以获得不错的效果.**
- 序列帧CSS  (逐帧CSS)
  - 基于设计师设置的缓动曲线和关键帧, 单个图层动画CSS可能会包含多个class以及多个keyframes。 需要用户将它们设置给不同的dom元素, 并且根据每个class的所执行的动画类型将它们分配给不同层级的dom, 从外到内的顺序是: translate->rotate->scale
...
对于这样的动效，我们本质上是要提取每个图层的动画参数，例如：transform、delay、duration等等，刚好AE提供了插件 [inspectorspacetime](https://google.github.io/inspectorspacetime/) ，可以把这些信息导出，利用这些信息，就可以根据设置的不同帧数和模式转换成 CSS 代码片段，开发者只需要粘贴相应的代码在项目中使用即可。具体可以参考：[图层转CSS动画](https://docs.corp.kuaishou.com/k/home/VNvtEEXf4k8s/fcABiIhxU3vtJ6uPUrOOWL04S)
...
国产的一种格式，https://svga.io/

SVGA 是一种同时兼容 iOS / Android / Flutter / Web 多个平台的动画格式。使用 After Effects 或是 Animate(Flash) 进行动画设计，SVGA 可以支持其中的大部分效果，设计师使用导出工具即可生成动画文件
...
首先 骨骼动画 是一种概念，就是指动画的效果是像我们实际物体是有骨骼的那样运动。那么你用任何手段达成这个效果都是可以的，比如css、js、canvas....

其次 最方便的骨骼动画的交付流程是 通过spine，即动效设计师 利用专业软件搞完动效后导出成 glTF 格式，然后开发直接同spine-player进行操作。
...
![img](https://static.yximgs.com/udata/pkg/EE-KSTACK/d0d98fa0f1e35f9ac1232ce43cfb9ac5.png)
...
不考虑设计、技术实现的成本，只考虑效果的话，有些动效的最佳分类是相对确定的：

- 【CSS动画】：用于无变形的过渡或简单动画。尤其是在硬件加速时。CSS不需要加载其他资源（一般指三方库），并且悬停时的小变换可以为交互带来更好的效果。特别是当你不需要3d、物理体感、或进行大量堆叠动画效果时建议选用CSS。另外，CSS方便调试也是很大的一个优势。

- 【骨骼动画】：人物、动物等，可以让动作更精准、逼真，研发的直接上游为「动效设计师」
- 【粒子系统】：一些大量单位（比如几百、上千个元素）规则变化的效果，研发的直接上游为「动效设计师」
- 【序列帧】视觉直接生产的一些播放类动效，比如开场动画等等，他们会使用 C4D 直接导出「序列帧」，研发的直接上游为「视觉设计师」
- 【Lottie】其他不好分类的动效，一般是动效设计师使用 AE 进行制作的，会使用 「Lottie」导出，研发的直接上游为「动效设计师」
...
勾选![img](https://docs.corp.kuaishou.com/image/api/external/load/out?code=fcABwO3OZX0SUux7lyhLPsDmM:3117712594325063844fcABwO3OZX0SUux7lyhLPsDmM:1694249031878)即可看到![img](https://docs.corp.kuaishou.com/image/api/external/load/out?code=fcABwO3OZX0SUux7lyhLPsDmM:7438838162739862407fcABwO3OZX0SUux7lyhLPsDmM:1694249031879)。
...
目前这部分也缺乏有效的线上数据，可以使用eva来收集实验室数据，参考：[如何使用Eva](https://docs.corp.kuaishou.com/k/home/VIox8otEGN9c/fcAAZy2xLEvkFlLa_z7895B4K)，目前虽然不支持 iOS 的内存数据获取，但Android的数据可以做个参考。
...
meta公司：https://github.com/facebook/lexical  [快手：**Lexical设计理念和在CMS应用**](https://docs.corp.kuaishou.com/k/home/VDM0ywyOHTeM/fcACELaJNHvd6vdE3pOPRUGbt)
